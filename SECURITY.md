...
- Updates are fetched from this repository's GitHub Releases over HTTPS and verified against `SHA256SUMS` before they are applied. A package with no trusted checksum is **refused**, and the checksum itself is only ever fetched from the official release source — mirrors may carry the bytes, never the proof.
- The Live Preview runs agent-generated HTML on a **separate loopback origin** that serves only static workspace files, so a previewed page cannot reach the local API or its token.
