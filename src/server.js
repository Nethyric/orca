...
async function handleApi(req, res, url) {
  ...
  if (p === '/api/send' && req.method === 'POST') {
    ...
    if (slash) {
      ...
      let reply = '';
      if (cmd === 'help') {
        const L = config.load().lang || 'en';
        const H = {
          en: ['**Commands**', '- `/init` — create `ORCA.md` (project memory, read in every chat of this folder)', '- `/compact` — compact this conversation', '- `/plan` — toggle plan mode (plan only, no file changes)', '- `/model <name>` — pick a model (`auto`, `minimax`, `deepseek`, `glm`, `glimmer`, or a model you added)', '- `/export` — export this chat as Markdown', '- `/files` — open the Files panel', '- `/clear` — new chat', '', '**Modes** — Chat, Compare (two models side by side), Plan, Web (research with citations). **Autonomy** — Ask / Auto / Full in the toolbar.', '**Sites & apps** — describe what you want; the result opens in the live Preview (phone / tablet / desktop / full-window).', '**Images** — attach a screenshot or photo; it is read by OCR and, when available, a vision model.', '', 'Press **F1** for the Help center, **Ctrl /** for shortcuts, **Ctrl K** for the command palette.'],
          fa: ['**دستورها**', '- `/init` — ساخت `ORCA.md` (حافظهٔ پروژه؛ در هر گفتگوی این پوشه خوانده می‌شود)', '- `/compact` — فشرده‌سازی این گفتگو', '- `/plan` — روشن/خاموش کردن حالت نقشه (فقط برنامه، بدون تغییر فایل)', '- `/model <نام>` — انتخاب مدل (`auto`، `minimax`، `deepseek`، `glm`، `glimmer` یا مدلی که خودتان اضافه کرده‌اید)', '- `/export` — خروجی Markdown این گفتگو', '- `/files` — باز کردن پنل فایل‌ها', '- `/clear` — گفتگوی جدید', '', '**حالت‌ها** — گفتگو، مقایسه (دو مدل کنار هم)، نقشه، وب (پژوهش با ارجاع). **خودمختاری** — Ask / Auto / Full در نوار بالا.', '**سایت و اپ** — بگویید چه می‌خواهید؛ نتیجه در پیش‌نمایش زنده باز می‌شود (موبایل / تبلت / کامپیوتر / تمام‌صفحه).', '**تصویر** — اسکرین‌شات یا عکس پیوست کنید؛ با OCR و در صورت وجود با مدل بینایی خوانده می‌شود.', '', '**F1** مرکز راهنما، **Ctrl /** میان‌برها، **Ctrl K** پالت دستورها.'],
          ru: ['**Команды**', '- `/init` — создать `ORCA.md` (память проекта, читается в каждом чате этой папки)', '- `/compact` — сжать этот диалог', '- `/plan` — режим плана (только план, без изменений файлов)', '- `/model <имя>` — выбрать модель (`auto`, `minimax`, `deepseek`, `glm`, `glimmer` или добавленную вами)', '- `/export` — экспорт чата в Markdown', '- `/files` — панель файлов', '- `/clear` — новый чат', '', '**Режимы** — Чат, Сравнение (две модели рядом), План, Веб (исследование со ссылками). **Автономность** — Ask / Auto / Full на панели.', '**Сайты и приложения** — опишите задачу; результат откроется в живом предпросмотре (телефон / планшет / компьютер / во весь экран).', '**Изображения** — прикрепите скриншот или фото; читается через OCR и, если доступна, через модель зрения.', '', '**F1** — справка, **Ctrl /** — горячие клавиши, **Ctrl K** — палитра команд.'],
          zh: ['**命令**', '- `/init` — 创建 `ORCA.md`（项目记忆，此文件夹的每个对话都会读取）', '- `/compact` — 压缩当前对话', '- `/plan` — 切换计划模式（只做计划，不改文件）', '- `/model <名称>` — 选择模型（`auto`、`minimax`、`deepseek`、`glm`、`glimmer` 或你添加的模型）', '- `/export` — 导出为 Markdown', '- `/files` — 打开文件面板', '- `/clear` — 新对话', '', '**模式** — 聊天、对比（两个模型并排）、计划、网页（带引用的研究）。**自主级别** — 工具栏中的 Ask / Auto / Full。', '**网站与应用** — 描述需求；结果会在实时预览中打开（手机 / 平板 / 电脑 / 全屏）。', '**图片** — 附加截图或照片；通过 OCR 和（可用时）视觉模型读取。', '', '**F1** 帮助中心，**Ctrl /** 快捷键，**Ctrl K** 命令面板。'],
        };
        reply = (H[L] || H.en).join('\n');
      }
